...
/*     Test the input parameters. */
	...
	*info = -4;
    ...
    /* WT: set iterations to zero earlier */
    *iter = 0;
...
/*        AV is temporary workspace here. */
    ...
    /* WT: &work[r * work_dim1 + 1]  contains  M^-1 (b - Ax)
           M^-1 is precon,  norm2 is norm of M^-1 (b - Ax)    */
    norm2 = dnrm2_(n, &work[r * work_dim1 + 1], &c__1);
    /* printf("norm2=%g bnrm2=%g tol=%g\n", norm2, bnrm2, tol); */
    /* WT: I commented out the next if-statement because it didn't make
       sense to me why norm2/bnrm2<tol is a good criterion for success. */
    /*
    if (norm2 / bnrm2 < tol) {
	goto L70;
    }
...
/*           tolerance, update the approximation vector X and quit. */
    ...
    *resid = (d__1 = work[i + 1 + s * work_dim1], abs(d__1)) / bnrm2;
    ...
    /* I changed GMRES.c so that when it exits because the number of
       iterations are exceeded, it exits in the same way as if the tolerance
       goal was reached (just the return value is different). Before it
       exited differently which always seemed to cause NANs. The original
       version is in GMRES_orig.c */
